...
function DockItem({
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      className={`relative inline-flex items-center justify-center rounded-full bg-white border border-ink/10 shadow-lg cursor-pointer z-20 ${className}`}
      tabIndex={0}
      role="button"
      aria-haspopup="true"
    >
    ...
    <AnimatePresence>
      {isVisible && (
        <motion.div
          ...
          className={`${className} absolute -top-10 left-1/2 w-fit whitespace-pre rounded-md border border-ink/10 bg-white px-2 py-1 text-xs text-ink font-medium shadow-sm transform -translate-x-1/2 z-30`}
          role="tooltip"
        >
          {children}
        </motion.div>
...
function DockIcon({ children, className = '' }: DockIconProps) {
  return <div className={`flex items-center justify-center ${className} text-ink`}>{children}</div>;
}
